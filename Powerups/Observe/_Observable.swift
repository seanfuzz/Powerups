...
class _Observable<T>
{
    typealias ObservableOutput = (_Observable<T>) -> Void
    
    var name = "Observable"
    
    var observers = [_Observer<T>]()
    
//    private var currentValue:T
    var value:T { didSet { output(self) } }
    
    var output: Closure<_Observable<T>> = { $0.broadcast() }
    
    var finished: Bool = false
    
    init(_ val:T, _ name:String? = nil, _ output:Closure<_Observable<T>>? = nil)
    {
        value = val
    ...
    @discardableResult
    func observe(_ update: Closure<T>? = nil) -> _Observer<T>?
    {
    ...
    func observe(start: Closure<T>? = nil,
                 ...
                 end: Closure<Error?>? = nil) -> _Observer<T>?
    {
        guard !finished else { return nil }
        
        let observer = _Observer(observable: self,
    ...
    func remove(observer: _Observer<T>)
    {
